        ...
        if (!empty($categories)) {
            foreach ($categories as $category) {
                if ($category->show_subcategory_products == 1) {
                    ...
                    $this->buildQuery();
                    $productsArray[$category->id] = $this->builder->whereIn('products.category_id', clrNum($category->id), false)->orderBy('products.created_at', 'DESC')->limit($limit)->get()->getResult();
                }
            }
        }
    ...
    public function increaseProductPageviews($product)
    {
        if (!empty($product)) {
            if (empty(helperGetCookie('pr_' . $product->id))) {
                ...
                $this->builder->update(['pageviews' => $product->pageviews + 1]);
            }
        }
    }
